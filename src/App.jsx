...
function App() { 
  ...
  const onPlaying = () => {
      ...
      // const n = audioElm.current.currentTime;
      // const m = audioElm.current.duration;

      //  curMin = Math.floor(m % (24 * 3600 * 3600) / 60)
      //  curSec = (Math.floor(n % (24 * 3600 * 3600 * 60) / 60)).toPrecision(2)

      //  totalMin= Math.floor(m % (24 * 3600 * 3600) / 60)
      //  totalSec = (Math.floor(m % (24 * 3600 * 3600 * 60) / 60)).toPrecision(2)
  ...
  return (
   <>
    ...
    {showList ?  <div className="w-72 mt-10 rounded-3xl h-full bg-slate-300 border border-gray-200 shadow dark:bg-black dark:border-gray-700 m-auto flex justify-center flex-col items-center top-10">

      <audio src={currentSong.audio} ref={audioElm} onTimeUpdate={onPlaying}></audio>
     ...
     </div> :  <div className="w-72 mt-10 rounded-3xl h-full bg-slate-300 border border-gray-200 shadow dark:bg-black dark:border-gray-700 m-auto flex justify-center flex-col items-center top-10">

     <audio src={currentSong.audio} ref={audioElm} onTimeUpdate={onPlaying}></audio>
    
      <Header showList={showList} setShowList={setShowList} />
      <Player curMin={curMin} curSec={curSec} totalMin={totalMin} totalSec={totalSec} songs={songs} setSong={setSong} isPlaying={isPlaying} setIsPlaying={setIsPlaying} currentSong={currentSong} setCurrentSong={setCurrentSong} audioElm={audioElm} />
